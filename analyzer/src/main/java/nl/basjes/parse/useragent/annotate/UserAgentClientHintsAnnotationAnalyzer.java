...
public class UserAgentClientHintsAnnotationAnalyzer<T> {
    private UserAgentClientHintsAnnotationMapper<T> mapper = null;
    private UserAgentClientHintsAnalyzer analyzer = null;
    private int cacheSize = DEFAULT_PARSE_CACHE_SIZE;
    private int clientHintsCacheSize = DEFAULT_PARSE_CACHE_SIZE;
    private static final Logger LOG = LogManager.getLogger(UserAgentClientHintsAnnotationAnalyzer.class);
    ...
    public void setClientHintsCacheSize(int newCacheSize) {
        clientHintsCacheSize = Math.max(newCacheSize, 0);
        if (analyzer != null) {
            analyzer.setClientHintsCacheSize(clientHintsCacheSize);
        }
    }
    ...
    public void initialize(UserAgentClientHintsAnnotationMapper<T> theMapper) {
        mapper = theMapper;
        ...
        analyzer = UserAgentClientHintsAnalyzer
            .newBuilder()
            .hideMatcherLoadStats()
            .withCache(cacheSize)
            .withClientHintsCache(clientHintsCacheSize)
    ...
    public T map(T record) {
        ...
        UserAgent userAgent = analyzer.parse(mapper.getRequestHeaders(record));
